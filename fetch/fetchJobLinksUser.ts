...
async function getJobSearchMetadata({ page, location, keywords }: { page: Page, location: string, keywords: string }) {
  ...
  await page.waitForSelector(selectors.keywordInput);
...
async function* fetchJobLinksUser({ page, datePosted = null, location, keywords, workplace: { remote, onSite, hybrid }, jobTitle, jobDescription, jobDescriptionLanguages }: PARAMS): AsyncGenerator<[string, string, string]> {
  ...
  while (numSeenJobs < numAvailableJobs) {
    ...
    // if (true) {  //targetIndex>0, Ensure there is at least one item and the index is within range
    //   await page.waitForSelector(`${selectors.searchResultListItem}:nth-child(${targetIndex})`, { timeout: 5000 });
    // } else {
    //   console.log("this error in fetchJobLinksUser line 95, all jobs viewed");
