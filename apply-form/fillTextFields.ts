...
async function fillTextFields(page: Page, textFields: TextFields): Promise<void> {
  ...
  for (const input of inputs) {
    ...
    if (!matched) {
      await changeTextInput(input, '', '0'); // default to 0 if no match is found
      unansweredQuestions.push(label);
    }
  }

  if (unansweredQuestions.length > 0) { // log unanswered questions to a file so use can update the textFields object in config
    fs.appendFileSync('unansweredQuestions.txt', unansweredQuestions.join('\n') + '\n');
  }
}
